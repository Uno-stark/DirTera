...
Icons are intentionally omitted from seed data; add them via the admin API.
"""
...
# ── Seed data — (slug, name, sort_order) ──────────────────────────────────────
...
# (slug, name, sort_order, parent_category_slug)
DOMAINS = [
    # ── Technology ────────────────────────────────────────────────────────────
    ...
    for slug, name, sort_order in CATEGORIES:
        conn.execute(
            text(
                ...
                "VALUES "
                "  (:id, :slug, :name, :sort_order, true, "
                "   CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) "
                "ON CONFLICT (slug) DO NOTHING"
            ),
            {"id": str(ULID()), "slug": slug, "name": name, "sort_order": sort_order},
        )

    for slug, name, sort_order, cat_slug in DOMAINS:
        conn.execute(
            text(
                "INSERT INTO domains "
                "  (id, slug, name, sort_order, category_slug, is_active, "
                "   created_at, updated_at) "
                "VALUES "
                "  (:id, :slug, :name, :sort_order, :cat_slug, true, "
                "   CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) "
                "ON CONFLICT (slug) DO NOTHING"
            ),
            {
...
def downgrade() -> None:
    ...
    domain_slugs   = [r[0] for r in DOMAINS]
    category_slugs = [r[0] for r in CATEGORIES]

    if domain_slugs:
        placeholders = ", ".join(f":d{i}" for i in range(len(domain_slugs)))
        conn.execute(
            text(f"DELETE FROM domains WHERE slug IN ({placeholders})"),
            {f"d{i}": s for i, s in enumerate(domain_slugs)},
        )

    if category_slugs:
        placeholders = ", ".join(f":c{i}" for i in range(len(category_slugs)))
        conn.execute(
            text(f"DELETE FROM categories WHERE slug IN ({placeholders})"),
            {f"c{i}": s for i, s in enumerate(category_slugs)},
        )
